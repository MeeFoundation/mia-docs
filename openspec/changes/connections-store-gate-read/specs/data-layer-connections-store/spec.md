...
Device-replicated registry of a user's connections: a dedicated iroh-docs replica, separate from data namespaces, that all devices of one identity converge on. Its entries pass the ingest gate via the device axiom (see `data-layer-ingest-policies`). Having the gate *read* this store to admit data is a deferred follow-up; this change covers the store and its replication between devices.

## ADDED Requirements

### Requirement: Connections live in a dedicated replica
Connection state SHALL be stored in its own iroh-docs replica, separate from every data namespace. The replica SHALL be bound in the node's registry as `Connections { owner }` (no domain `NamespaceId`); data namespaces keep the `(about, issued_by)` pair unchanged.

#### Scenario: Creating the store binds it by kind
- **WHEN** a node creates its connections store
- **THEN** a fresh iroh-docs replica is created and bound as `Connections { owner: <local PdnId> }`, and no domain `NamespaceId` is allocated for it
...
`disconnect(P)` SHALL write an iroh-docs tombstone (empty entry, length 0) at `connections/<P-hex>`. A connection SHALL be considered live if and only if the latest entry for its key across all authors has non-zero length.
...
Connection mutations performed on one device SHALL become visible on the identity's other devices through standard iroh-docs sync (set reconciliation for catch-up, gossip for live updates), with no additional transport or server.
...
Concurrent mutations of the same connection key on different devices SHALL converge on every device to the entry with the newest timestamp (iroh-docs per-key LWW across authors), with tombstones participating like ordinary entries.
