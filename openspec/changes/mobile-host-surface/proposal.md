...
- **A single entry payload is bounded.** A write buffers the whole payload across the binding boundary, and on a phone an unbounded one ends the process rather than returning an error. Memory pressure is the one operating condition a phone adds that a container never had, so the facade bounds what it controls and says so.
...
| `components/pdn-sdk/artifacts`      | `openspec/specs/components/pdn-sdk/artifacts/spec.md`       |

### New Capabilities

- `components/mee-pdn/pdn-mobile/host`: what the facade exposes and what it withholds, how entry bytes and paths cross it, what a read that answers nothing means, how a refusal is reported, that one handle owns one node, and what it says about the state a node keeps and an identity without keys.
- `components/pdn-sdk/artifacts`: what a release is built from and names, that the packaging adds nothing to the facade's surface and holds no source of its own, that generated bindings are committed nowhere, the configuration and the architectures a release carries, and that an application consumes a release by naming it.
...
- **`components/mee-pdn/data-layer/node-assembly/spec.md`**: gains the periodic reconcile pass, the interval its spawn names and the 10-second default as a requirement, so a mobile specification is not the only written statement of the number a host configures. What a node keeps and where is `durable-storage/spec.md`'s and needs no move — it states already that storage is named at the spawn and that neither mode is a default.
