...
- `mobile-common-host-surface`: what the facade exposes and what it withholds, how entry bytes and paths cross it, what a read that answers nothing means, how a refusal is reported, that one handle owns one node, and what it says about the state a node keeps and an identity without keys.
