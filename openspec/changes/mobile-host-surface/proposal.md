...
- `components/mee-pdn/pdn-mobile/host`: what the facade exposes and what it withholds, how entry bytes and paths cross it, what a read that answers nothing means, how a refusal is reported, that one handle owns one node, and what it says about the state underneath it and an identity without keys.
