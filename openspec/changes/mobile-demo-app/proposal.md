...
The audience fixes the form. Every act whose subject is what a person sees is watched on a screen — the phone, or a browser page running the same screens over a counterparty node's state — and neither is narrated as something it is not: a terminal never stands in for a screen, and a browser screen is named for the node it shows rather than mistaken for a second phone. And nothing is claimed that the code does not do, because a demonstration that oversells is discovered later by the same audience. What the platform genuinely lacks — an identity backed by key material — is stated rather than avoided, and so is what a staging with one physical phone cannot show.
...
- **Both directions of granting are watched, each on the screen of its issuer.** Alice's phone is the only node a person taps acts into; Bob's granting still runs through the run-through's script, but a browser page running the same screens shows its effect on his node, so the acts of an issuer — choosing a claim, publishing, withdrawing, granting again — are watched twice over: tapped through on Alice's phone when she grants Bob, and shown on that browser screen when Bob's script grants Alice. What each grantee obtains — a claim arriving, a writable claim accepting an edit, a read-only claim refusing one, a withdrawn claim leaving the screen as no longer shared — is read on the other party's own screen in turn, never narrated from a terminal instead.
- **A grant reaches every device of the identity it names, not the device that first read it.** Alice's laptop node, which took no part in reading Bob's grant the first time, already reads the claims it names and picks up a later change to one of them, with no act performed there and no phone involved at all — the same property act 3 already showed for a connection, now shown for what a grant carries.
