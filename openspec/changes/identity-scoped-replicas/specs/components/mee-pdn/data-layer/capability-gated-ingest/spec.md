...
The caller's write set SHALL be computed at session setup from the issuer's recorded grants toward the identity the caller names — the same resolution and the same records the read side uses — and SHALL hold for that session's lifetime. It SHALL be recorded per scope as well as per replica and caller, so a node hosting several identities granted by one issuer is admitted, in each session, exactly what the identity named there was granted.
