...
When enabled, the debug surface SHALL make identity creation and linking, connection establishment, grants, entry operations, node status, and hosted identities reachable over HTTP. An entry operation SHALL name the identity performing it as well as the issuer addressed, so a caller reaches exactly what that identity's scope holds. Each route SHALL delegate to a runtime service call and add no orchestration of its own.
