...
- **A container spawned from inside the development container is a sibling, not a child** → its published port lands on the host's loopback, and the development container runs in another network namespace, so the address the harness dials must come from the container client's own host resolution (the override it reads for exactly this case) rather than from an assumed loopback. From the host the same resolution is loopback and nothing is needed. The suite therefore runs in both places with nothing set by hand.
