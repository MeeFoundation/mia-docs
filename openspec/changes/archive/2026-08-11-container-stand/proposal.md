...
- **Authentication on the debug surface** — where the surface is published is its whole boundary, and that is an arrangement rather than a security boundary. The flag keeps the surface absent by default, the demo publishes on loopback, and a suite run publishes each node's port to the machine running it; on the stand's own network every node reaches every other's surface. What that exposes is throwaway: nodes off the product path, holding what the scenario just wrote and gone with it, while a product host embeds the runtime in its own process and serves no HTTP at all. A control plane that authenticates its caller is its own change, and neither it nor its absence moves the runtime's own boundaries, which are the capability and the device set.
