...
- **The store becomes a crate of the workspace, `crates/pdn-store`.** A copy of the checkout at its current revision — source, tests, examples, property-test regressions, licenses, README — with nothing of its code, its tests, or its docs changed but one loop in a test, rewritten for a lint clippy 1.98 adds (`for_unbounded_range`, on the endless ticker of `sync_big`) — the pipeline's floating stable toolchain met it on the first run. The package keeps the upstream name `iroh-docs` and the workspace alias `pdn-store` stays, so no `use` line moves anywhere; the manifest gains `publish = false` and a description of what the crate is, and loses the block that addressed docs.rs. The README's head names the variant in place of upstream's badges.
...
| -------------------------- | ------------------------------------------------------- |
| `data-layer-store-crate`   | `openspec/specs/components/mee-pdn/pdn-store/crate.md`   |
...
- **`crates/pdn-store`** (new): 42 files copied from the checkout at revision `721de62`; `src/`, `examples/`, `proptest-regressions/`, `build.rs`, and both licenses byte for byte; `tests/sync.rs` with the one loop rewritten; `Cargo.toml` metadata and the README's head as described.
