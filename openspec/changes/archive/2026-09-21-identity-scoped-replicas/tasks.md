...
- [x] 3.11 A directory or a connection metadata store imported for an identity is tracked at the import and starts its sync once armed — by hosting the identity or the connection (D17); and hosting a connection points its own half at the devices its peer half is dialed at; verified by `both_halves_of_a_co_located_pair_converge_whichever_side_arms_first`, which fails with the sync started at the import and with the own half left unpointed, and by `a_co_located_establishment_reads_the_counterpartys_devices_at_once` on the product path
