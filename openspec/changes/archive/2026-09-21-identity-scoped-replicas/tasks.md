...
- [x] 3.6 The replica store's cache bound is a share cut at spawn from a node budget and the count of identities the device is provisioned for, with one default of 1 GiB and one identity declared once and taken by the hosts and the suites (D14); verified by a spawn without either value bounding its one identity at the whole budget, a spawn with both bounding each identity at budget over count, an identity created on a running node opening at the same share with no other store reopened, and a node that outgrows its count reporting it
