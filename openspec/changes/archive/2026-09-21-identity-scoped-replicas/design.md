...
A node's directory keeps its endpoint key and its lock where they are, and holds one subdirectory per hosted identity with that identity's replica store, its author and its hosting record. The blob store stays one directory for the node.

The hosting record is one small file in the identity's subdirectory holding the namespace of its private metadata directory, and the subdirectory's name is the identity. It is the one fact about an identity that nothing else on disk answers: the store holds several of the identity's replicas, and the directory's namespace is random, so no reading of the store tells which replica is the directory. The record is written at the commit point of the create or link that hosts the identity, after the identity's replicas are flushed, beside and renamed over, and recording one identity never rewrites another's. A start re-hosts every identity whose subdirectory holds a record, and a subdirectory without one — what an unfinished create or link left — hosts nothing. A record whose replica store is gone from the subdirectory is skipped before anything is provisioned for it, so the skip opens no store in its place.

The record takes the place of the node-wide file that disk-persistence D4 and D5 decided, and keeps what those decisions rest on: the directory's namespace is the only fact written down, everything else re-derives from the directory along the product path, and the record is the last step of a create or link that can fail, so an identity is either recorded and hosted or neither. An unreadable record stops the start, naming its file, and a directory with no record is a first start. No operation ends an identity's hosting; one that did would remove that identity's record and touch no other. A subdirectory without a record is the positive marker disk-persistence left open for collecting the leftovers of an interrupted create or link: every replica an identity holds lives in its subdirectory, so everything in one without a record belongs to an attempt that never committed.

The subdirectories with a record are also what a store's share of the cache budget is counted from (D14), so the layout is read as well as written.

**Rejected alternatives:**

- One record of the hosted identities for the whole node, `hosted-identities.json`, as disk-persistence D4 decided.
...
- **A device that restarts** changes the outcome: each hosted identity comes back with its own store and author from the hosting record in its own subdirectory, a record whose store is gone is skipped without opening one and stays for the next start, and contacts that name this node's own address route to the in-process path rather than failing to dial (D7, D10, D12).
...
- **A disk that fills** is affected in degree: a namespace held by two identities of one node is stored twice, and the storage a node needs grows with the identities it hosts, not with the namespaces alone. A hosting record the disk refuses at the commit point fails that create or link and leaves no record, so the identity comes back at no start and the identities already recorded are untouched (D12).
