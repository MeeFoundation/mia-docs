...
- [x] 3.4 The paired denial in two degrees (D4): a second identity hosted on the same runtime with no pair toward that peer obtains nothing — the degree a plain peer-keyed lookup falls to — and a second identity that does hold its own connection to the same peer reads its own grants and never the first identity's, the degree that reaches the slip the other cannot
- [x] 3.5 Each scenario proven to fail with its mechanism deliberately broken. For a denial whose expected answer is empty, the discriminating half is the positive read beside it: an implementation that answered nothing to everyone would pass the denial alone
  - Second broken arm, for the denials: the pair resolved by peer alone rather than through the acting identity's directory. All 3 scenarios red, each on its first-degree co-hosted denial — publishing caches the pair, so that degree is where a plain peer-keyed lookup lands. A third arm, the same lookup restricted to identities that already hold a pair, reaches the second degree: 2 of 3 scenarios green, the first red on the claim set X reads back.
