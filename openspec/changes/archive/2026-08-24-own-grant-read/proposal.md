...
- **It answers for the device it runs on, and one empty answer covers three states.** The read reports what this device holds: on the device that published a grant it is evidence the record is here, never that it reached a sibling or the peer. And an empty answer means one of "no connection to that peer", "the pair has not replicated here yet" and "nothing is granted" — the peer-side read already answers that way for the same reason, and a host that presented it as the fact that nothing is shared would say the one thing the read cannot support.
