...
Tighter still, and worth having beside it: a second identity that does hold its own connection to the same peer. Both identities then have a pair toward P, and a lookup keyed on the peer alone rather than on the acting identity's directory would hand one identity the other's grants — a bug the empty-directory case cannot catch. It also supplies what an empty expectation lacks: a denial whose expected answer is nothing is satisfied by an implementation that answers nothing to everyone, so the assertion that discriminates is the positive read standing beside it.
