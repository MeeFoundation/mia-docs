...
Scope: the cell as a platform primitive — a keyless id, one cell store replicated whole among member devices, authorship-judged admission, membership with owners joined under a one-time secret, claims and documents — mergeable-documents and immutable-documents — as content. Nothing existing changes shape. Group 0 is the gate: the design's blocking open questions are answered and recorded before any implementation task starts.
...
- [ ] 0.7 Record the answers as design decisions D19 and following, and rewrite the affected scenarios in the specs before implementation starts
...
- [ ] 1.3 `pdn-layer`: the record kinds — claim, document with its type, mergeable-document or immutable-document (D17) — and the key layout of a cell store per the answers to C2 and C4
...
- [ ] 2.5 Removal: sessions refused and the removed member's authors dropped from the first session set up after the removal record arrives; what was delivered is retained — the member's own records and its operations on other members' documents alike; a member that joins again is admitted from its new membership record, its new writes persisted; an entry that arrives ahead of the membership record authorizing its author is dropped and persisted from the next session (D8' may reorder the session)
