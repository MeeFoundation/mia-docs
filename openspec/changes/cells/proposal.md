...
- **A new primitive, the cell.** A space shared by 0..n members, identified by a cell id that carries no key material: the id is derived from its creator's announcement key and a random nonce, so every device checks the root of membership against the id it holds. A cell signs nothing, issues nothing and proves nothing; every act inside it is a member's act. One cell is two pdn-store replicas — the **membership store**, holding who is a member, with what role, on which devices, and the **record store**, holding the records — both held whole by every device of every member.
