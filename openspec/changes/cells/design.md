...
- F8. Hash-linked event sequences: each membership event carries the hash of the previous event in the sequence it extends, so that a rewrite or a back-insertion shows as a fork on a device that never held the original (F4), and an actor's own sequence commits to its acts — the closing of F7. Not hand-rolled ahead of KERI: an actor's log that commits to its own acts, and the detection of one actor contradicting itself, come with KERI's key event log (F4). What KERI does not resolve is a fork written by several actors — two owners acting on one member at one sequence (B10). Both events are valid and cite the same predecessor; KERI's duplicity handling covers one controller contradicting its own log and gives no order across different controllers' logs, so that fork keeps the deterministic rule B10 settles, reached from the events alone in any arrival order.
