...
Every member device holds both stores whole and holds their write tickets; write authority inside a cell is the gate's, judged per entry (D6, D12, D17), never the ticket's mode. Any member edits other members' mergeable-documents (D6), an owner deletes other members' records (D12), and every member relays what it holds (D7) — none of which a read-only ticket allows, and a ticket per role would have to be re-cut at every role change (D11). A removed member keeps the write tickets it held: honest devices refuse its sessions and drop the entries it authors under a sequence at which it was no longer a member (D13, D22), which is the whole of removal under bearer tickets — an entry it authors afterwards under an earlier sequence is the retrograde case F7 leaves open; UWill takes write authority out of the ticket (F1, F3).
