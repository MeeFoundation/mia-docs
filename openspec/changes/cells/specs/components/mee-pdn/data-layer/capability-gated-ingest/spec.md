...
A session peer resolving as a device of the issuer SHALL be admitted in full. The gate SHALL arm on replicas data-bound to a hosted identity and on a cell's two stores. On a hosted issuer's data replica it judges by the session peer's write admission, as above. On a cell's record store it judges by the entry's author key, resolved to a member through the cell's member device records — each hosted identity writes with its own author, so a key resolves to one member — and by that member's membership state at the membership sequence the entry's key names (cells D22), never by the session peer — a member device relays other members' entries, so the peer that sends an entry is not its author — admitting a claim or an immutable-document from a device of the member under whose name it sits, a mergeable-document's operation from any member's device, a tombstone from a device of the record's member or of an owner, and dropping every other entry silently, as the [cell stores](../cell-store/spec.md) spec states. On a cell's membership store it judges the membership material as that spec states: a device-list statement by the signature embedded in it, against the member's announcement key, with the entry's author ignored; a removed, made-owner or unmade-owner event by its actor being an owner at the actor's sequence the key names, a joined event by its actor being a member there, a left event by its actor being the subject, the founding event by deriving the cell id and carrying a valid signature under the announcement key it names (cells D23, D25); and an entry under a subject sequence its author already holds is dropped. Directories and connection metadata stores keep ticket-bounded admission (Invariants 1 and 3), and a grantee-held replica of a foreign namespace admits what the serving side's egress delivers. Retraction markers SHALL be consulted on data replicas only, the only replicas whose entries a marker can name, so no state of the marker set can reach the stores that carry device records and grants, nor a cell's stores.
