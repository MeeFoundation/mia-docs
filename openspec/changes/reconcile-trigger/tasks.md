...
- [ ] 1.1 On a write, resolve the scoped peers whose capabilities cover the written claim (reusing subset-rbsr's capability / audience index) and send each a content-free trigger over its direct connection, addressed to the node and the identity holding the replica there — a node holding one namespace for two identities receives one trigger per covering identity
...
- [ ] 1.4 An addressed identity hosted on the sending node is triggered inside the process, as its reconciliation already runs there, and no co-located trigger leaves the node
...
- [ ] 3.4 Flake check on the new scenarios; lints + full suite (`just precommit-check`)
