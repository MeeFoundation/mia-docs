...
* **Inviter-side registration**: after the burn, the inviter writes the newcomer's device record into its own directory replica, taking the node id from the connection's authenticated peer identity — never from a claimed field; the request carries none. The registration is a local write on a device that already holds the directory, so no cross-node delivery sits in the linking critical path: the confirmation loop's race is removed, not re-cured. The identity's other devices learn of the newcomer through ordinary directory replication.
...
* Neutral — a reply lost after commit leaves a registered-but-absent device record: harmless (device records carry no liveness semantics), converging on the next fresh invite — the same lost-reply posture as re-establishment.
