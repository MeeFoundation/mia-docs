...
* **After the dialogue**: each side records the connection in its connections store and imports the counterpart's ticket. Everything later — capability grants, data-store tickets, further sharing, revocation — travels *inside* the exchanged metadata replicas over ordinary pdn-store sync, and reaches both identities' other devices through device replication. The raw channel is used once per establishment; it is not a data path.
