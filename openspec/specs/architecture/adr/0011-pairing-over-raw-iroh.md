...
* **The QR carries**: the inviter device's node address (endpoint public key plus every address the endpoint publishes about itself: direct socket addresses, and a relay URL on a runtime that binds relays — a spawn-time choice that is off by default, so a QR minted by the suites and by the container stand carries direct addresses alone), a one-time short-lived pairing secret (random, not a counter), the inviter's PdnId, and a format version. It carries **no** tickets and **no** identity proof.
