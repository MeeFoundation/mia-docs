...
The `data-layer` crate's `SyncNode` over the variant: it supplies the validator at spawn (`capability_ingest_validator` in `src/access.rs`, installed by `SyncNode::spawn` in `src/node.rs`), rejects entries outside the sender's recorded write set **at the `validate_entry` / `validate_cb` hook** under adversarial test inputs (rejected entries are *absent* from the store, not merely unpromoted), admits valid entries by auto-merge, delivers grants over the connection metadata store ahead of the data they authorize, and answers a later withdrawal by refusing the next session and retracting on the sender's side ([write retraction](../../components/mee-pdn/data-layer/write-retraction/spec.md)) — an entry the issuer accepted while the grant covered it stays. Compliance criterion: **no entry is stored that the validator refuses** — an entry outside the sender's recorded write set is absent from a hosted issuer's replica, not merely unpromoted. The scenarios are `read_restriction.rs` in `crates/data-layer/tests` and `scoped_writes.rs` in `crates/pdn-node/tests`.
...
2. **Revocation after admission** — retraction markers refuse the exact entries a withdrawal covers, and a rejected sender retracts its own copy on the echoed verdict ([write retraction](../../components/mee-pdn/data-layer/write-retraction/spec.md)).
3. **Capability delivery** — grants travel in the [connection metadata store](../../components/mee-pdn/data-layer/connection-metadata-store/spec.md) ahead of the data they authorize, and a session's write set is resolved from them at setup.
