...
# UWill: UCAN-shaped capabilities

## Context and Problem Statement

Due to the earlier design decision (not captured here), PDN uses [capability-based security](https://en.wikipedia.org/wiki/Capability-based_security). Each [claim](../language/claim.md) shared with another [person](../language/person.md) has an associated [capability](../language/capability.md) that specifies what can be done with that claim in the scope of a specific [connection](../language/connection.md). Initially, PDN capabilities were modelled after Willow's [Meadowcap](https://willowprotocol.org/specs/meadowcap/index.html#meadowcap) system. But the capability mechanism must be resistant to retrograde attacks and hence should use [anchored signatures](https://dhh1128.github.io/papers/was.html). In that light, Meadowcap has some deficiencies:
...
4. **No identity-system integration.** Capabilities reference raw ed25519 keys and are tied to specific physical willow nodes — but a Mee participant has multiple devices, and capabilities must reference PDN identities, per [ADR-0004](0004-capabilities-should-refer-to-mee-identity.md).
...
- **UWill: hybrid UCAN delegation chain + Willow Area policy, flat command set, bound to PdnId.** ← chosen
...
- Bad, because capabilities stay tied to physical nodes, violating [ADR-0004](0004-capabilities-should-refer-to-mee-identity.md).
...
- Bad, because it contradicts [ADR-0004](0004-capabilities-should-refer-to-mee-identity.md).
