...
- **WHEN** `data-layer` opens an issuer's data namespace for a hosted identity
- **THEN** it is keyed by the issuer `PdnId` alone, with no `(about, issued_by)` pair

#### Scenario: one namespace, one replica per identity holding it
