...
Capability-filtered reconciliation — "subset-RBSR" — the read-side counterpart of the ADR-0008 ingest hook, enforcing Invariant 2: a serving node reveals only the claims the receiving peer is read-authorized for. The grant vocabulary the filter consumes is the [read capabilities](../read-capabilities/spec.md) mechanism.
