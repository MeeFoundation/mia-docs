...
Device-replicated registry of a user's connections: a dedicated pdn-store replica, separate from data namespaces, that all devices of one identity converge on. Its entries pass the ingest gate via the device axiom (see [ingest-policies.md](ingest-policies.md)); having the gate *read* this store to admit data is a deferred follow-up. This spec covers the store and its replication between devices.
