...
Domain-level admission policies in `data-layer`: how the fork's validator callback (`Fn(&SignedEntry) -> bool`) is bridged into domain terms, and which policy decides. In this scope the only policy is Invariant 1 (`SelfOwned`), which inspects the resolved binding and reads no store. (Having a policy *read* the connections store to gate data is a deferred follow-up.)
...
- **THEN** the policy sees `Connections { identity }`
...
#### Scenario: Foreign binding is not admitted by Invariant 1
- **WHEN** an entry arrives whose binding is owned by a different identity
- **THEN** Invariant 1 does not admit it

### Requirement: Policies compose
Policies SHALL be composable such that an entry is admitted when any composed policy admits it (first-accept). The policy set provided in this change (Invariant 1 alone) SHALL reject entries of unbound replicas and of bindings not owned by the local identity.
