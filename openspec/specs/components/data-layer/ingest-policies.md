...
Domain-level admission policies in `data-layer`: how the fork's validator callback (`Fn(&SignedEntry) -> bool`) is bridged into domain terms, and which policy decides. In this scope the only policy is the device axiom (`SelfOwned`), which inspects the resolved binding and reads no store. (Having a policy *read* the connections store to gate data is a deferred follow-up.)
